...
### Что получится в `out/report.md` (результат)

После `compile` файл будет **обычным Markdown** — таким (содержимое буквально):
...
_Сгенерировано relator._
```

Сначала подставились `[[title]]` и `[[TABLE.rows]]`, затем вместо `@@footer@@` вставился текст слота. В браузере или в превью Markdown это отображается как заголовок, таблица и курсивная строка.
