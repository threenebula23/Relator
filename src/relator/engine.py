...
from .builtins import resolve_cell, resolve_item, resolve_json_namespace, resolve_list_namespace, resolve_media_namespace, resolve_table_namespace, resolve_tree_namespace, resolve_var_special
from .errors import TemplateError
from .parser import LenBlock, RenderBlock, find_first_struct_block
...
def resolve_placeholder(placeholder: str, context_stack: list[Mapping[str, Any]], loop_stack: list[LoopInfo], root_context: Mapping[str, Any], assets_dir: Path, template_kind: str, sql_dialect: str | None=None) -> str:
    ...
    if first_upper == 'TREE':
        return resolve_tree_namespace(parts[1:], root_context)
    if first_upper == 'JSON':
        return resolve_json_namespace(parts[1:], root_context)
...
def render_template(template: str, root_context: Mapping[str, Any], assets_dir: Path, template_kind: str, context_stack: list[Mapping[str, Any]] | None=None, loop_stack: list[LoopInfo] | None=None, sql_dialect: str | None=None) -> str:
    ...
    loops = loop_stack or []
    first = find_first_struct_block(template)
    if first is None:
    ...
    if kind == 'render':
        ...
        rb = block
        prefix = _join_without_linebreaks(lines[:rb.start_line])
        suffix = _join_without_linebreaks(lines[rb.end_line + 1:])
        rendered_body = render_template(template=rb.body, root_context=root_context, assets_dir=assets_dir, template_kind=template_kind, context_stack=ctx_stack, loop_stack=loops, sql_dialect=sql_dialect)
        merged = prefix + rendered_body + suffix
        return render_template(merged, root_context, assets_dir, template_kind, ctx_stack, loops, sql_dialect)
    assert isinstance(block, LenBlock)
    len_block = block
    prefix = _join_without_linebreaks(lines[:len_block.start_line])
    suffix = _join_without_linebreaks(lines[len_block.end_line + 1:])
    if len_block.var_name not in root_context:
        raise TemplateError(f"Unknown loop variable '{len_block.var_name}' in %%len=...%% block.")
    seq_raw = root_context[len_block.var_name]
    if isinstance(seq_raw, (str, bytes)) or not hasattr(seq_raw, '__len__'):
        raise TemplateError(f"Loop variable '{len_block.var_name}' must be sequence-like.")
    seq = list(seq_raw)
    rendered_body_parts: list[str] = []
    for index, item in enumerate(seq):
        local_scope = {len_block.var_name: item} if isinstance(item, Mapping) else {len_block.var_name: item}
        loop = LoopInfo(var_name=len_block.var_name, seq=seq, index=index)
        rendered_piece = render_template(template=len_block.body, root_context=root_context, assets_dir=assets_dir, template_kind=template_kind, context_stack=[local_scope, *ctx_stack], loop_stack=[*loops, loop], sql_dialect=sql_dialect)
